package com.Stoffel.snakeColor;

public class Window {

	public static final int IOS_WIDTH = 800;
	public static final int IOS_HEIGHT = 480;
	
	public static final int DESKTOP_WIDTH = 800;
	public static final int DESKTOP_HEIGHT = 480;
	
	public static final int WEB_WIDTH = 800;
	public static final int WEB_HEIGHT = 480;
	
	public static final int ANDROID_WIDTH = 800;
	public static final int ANDROID_HEIGHT = 480;
	
	public static final int WIDTH = 800;
	public static final int HEIGHT = 480;
	
	
	
}
